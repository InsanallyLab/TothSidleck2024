...
if __name__ == "__main__":
    ...
    #Check for existing ensemble list
    must_generate_enum_lists = True
    EnumSessionFilename = os.path.join(OUTPUT_DIRECTORY,'EnumSession.pickle')
    EnumEnsemblesFilename = os.path.join(OUTPUT_DIRECTORY,'EnumEnsembles.pickle')
    if os.path.isfile(EnumSessionFilename) and os.path.isfile(EnumEnsemblesFilename):
        must_generate_enum_lists = False

    if not must_generate_enum_lists:
        try:
            with open(os.path.join(EnumSessionFilename), 'rb') as f:
                EnumSession = pickle.load(f)
        except Exception as e:
            print(f"run_dimm line 68: {e}")
        try:
            with open(os.path.join(EnumEnsemblesFilename), 'rb') as f:
                EnumEnsembles = pickle.load(f)
        except Exception as e:
            print(f"run_dimm line 73: {e}")

    ####################################################################################################################################

    if must_generate_enum_lists:
        EnumSession = []
        EnumEnsembles = []
        ...
        for session in sessions:
            ...
            try:
                ensembles = ilep.getPossibleCombinationsAZ(sessionfile,trialsPerDayLoaded,minClust=ENSEMBLE_SIZE,MaxClust=ENSEMBLE_SIZE,MaxSample=len(sessionfile.clusters.good)*SAMPLING_COEFFICIENT,MaxIter=MAX_ITERS)
    ...
    for session,ensemble in progress_bar:
        ...
        try:
            ...
            temp = pool.apply_async(ilep.calculateDimensionalityParallel,(session,CACHE_DIRECTORY,OUTPUT_DIRECTORY,ensemble,trialsPerDayLoaded,BIN_LENGTH,200))
