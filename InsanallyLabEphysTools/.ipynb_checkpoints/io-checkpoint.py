...
from .utility import generateSaveString
...
def loadSessionInitial(DIRPATH,NAME,region):
    ...
    if session.meta.task in ['nonreversal','switch','reversal','opto nonreversal','opto switch','opto reversal','tuning nonreversal','tuning switch','tuning reversal']:
        try:
    ...
    if session.meta.task in ['opto nonreversal','opto switch','opto reversal']:
        try:
            ...
            print(e)
    #Load trimming data
    if session.meta.task in ['nonreversal','switch','reversal','opto nonreversal','opto switch','opto reversal','tuning nonreversal','tuning switch','tuning reversal','passive no behavior']:
        try:
    ...
    session.meta.lickdelayrelativetostart = 0.2
    ...
    for trial in range(session.meta.length_in_trials):
        #Nonreversal
        if session.meta.task in ['nonreversal','opto nonreversal','tuning nonreversal'] or (session.meta.task in ['switch','opto switch','tuning switch'] and trial < session.meta.first_reversal_trial):
        ...
        #Reversal
        elif session.meta.task in ['reversal','opto reversal','tuning reversal'] or (session.meta.task in ['switch','opto switch','tuning switch'] and trial >= session.meta.first_reversal_trial):
